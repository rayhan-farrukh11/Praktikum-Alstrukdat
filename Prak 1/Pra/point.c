...
boolean EQ (POINT P1, POINT P2) {
    return (Absis(P1) == Absis(P2) && Ordinat(P1) == Ordinat(P2))? true : false;
};
...
boolean IsOrigin (POINT P) {
    return (Absis(P) == 0 && Ordinat(P) == 0) ? true : false;
};
/* Menghasilkan true jika P adalah titik origin */

boolean IsOnSbX (POINT P) {
    return (Ordinat(P) == 0) ? true : false;
};
/* Menghasilkan true jika P terletak Pada sumbu X */

boolean IsOnSbY (POINT P) {
    return (Absis(P) == 0) ? true : false;
};
...
POINT NextX (POINT P) {
    POINT Pnew;
    CreatePoint(&Pnew,Absis(P)+1,Ordinat(P));
    return Pnew;
};
/* Mengirim salinan P dengan absis ditambah satu */              
POINT NextY (POINT P) {
    POINT Pnew;
    CreatePoint(&Pnew,Absis(P),Ordinat(P)+1);
    return Pnew;
};
...
POINT PlusDelta (POINT P, float deltaX, float deltaY) {
    POINT Pnew;
    CreatePoint(&Pnew,Absis(P)+deltaX,Ordinat(P)+deltaY);
    return Pnew;
};
...
POINT MirrorOf (POINT P, boolean SbX) {
    POINT Pnew;
    SbX ? CreatePoint(&Pnew,Absis(P),Ordinat(P)*(-1)) : CreatePoint(&Pnew,Absis(P)*(-1),Ordinat(P));
    return Pnew;
};
...
void Mirror (POINT *P, boolean SbX) {
    SbX ? CreatePoint(P,Absis(*P),Ordinat(*P)*(-1)) : CreatePoint(P,Absis(*P)*(-1),Ordinat(*P));
}
/* I.S. P terdefinisi */
...
void Putar (POINT *P, float Sudut) {
    float radian = Sudut*3.14159265358979323846/180;
    float sin = sinf(radian);
    float cos = cosf(radian);

    Absis(*P) = (Absis(*P) * cos) - (Ordinat(*P) * sin);
    Ordinat(*P) = (Absis(*P) * sin) + (Ordinat(*P) * cos);
};
/* I.S. P terdefinisi */
